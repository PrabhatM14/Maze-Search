...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    pos = currentPos
    while unvisited:
        distances = [util.manhattanDistance(pos, c) for c in unvisited]
        minDist = min(distances)
        h += minDist
        idx = distances.index(minDist)
        pos = unvisited.pop(idx)
    return h
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if not foods:
        return 0
    far = max(foods, key=lambda c: util.manhattanDistance(position, c))
    if 'distances' not in problem.heuristicInfo:
        problem.heuristicInfo['distances'] = {}
    key = (position, far)
    if key not in problem.heuristicInfo['distances']:
        problem.heuristicInfo['distances'][key] = mazeDistance(position, far, problem.startingGameState)
    return problem.heuristicInfo['distances'][key]
